...
export const HospitalFilter = ({
  ...
  return (
    <div className="bg-white p-4 sm:p-5 rounded-3xl border border-slate-200/80 shadow-md space-y-4 sm:space-y-5">
      <div className="flex items-center justify-between border-b border-slate-100 pb-2.5">
        ...
        <button
          onClick={onReset}
          className="text-xs font-semibold text-blue-600 hover:text-blue-800 flex items-center gap-1 transition-colors cursor-pointer min-h-[36px] px-1"
        >
      ...
      <div>
        <label className="block text-[11px] sm:text-xs font-bold uppercase text-slate-500 mb-1">
          City / Location
        </label>
        <select
          value={filters.city || "Near Me"}
          onChange={(e) => onChange({ ...filters, city: e.target.value })}
          className="w-full px-3 py-2 text-xs sm:text-sm bg-slate-50 border border-slate-200 rounded-xl font-semibold text-slate-800 focus:outline-none focus:border-blue-500 cursor-pointer min-h-[42px]"
        >
      ...
      <div>
        <div className="flex items-center justify-between text-xs font-bold text-slate-700 mb-1">
          ...
          <span className="text-blue-600 bg-blue-50 px-2 py-0.5 rounded-md border border-blue-100 font-extrabold text-[11px] sm:text-xs">
      ...
      <div className="pt-2 border-t border-slate-100">
        <button
          ...
          className="w-full min-h-[42px] py-2 px-3 bg-slate-50 hover:bg-slate-100 active:bg-slate-200 text-slate-700 font-bold text-xs rounded-xl border border-slate-200 flex items-center justify-between transition-colors cursor-pointer"
        >
          <span className="truncate pr-1">{showMoreFilters ? "Fewer Filters" : "More Filters (Insurance & Services)"}</span>
          {showMoreFilters ? <ChevronUp className="w-4 h-4 text-slate-500 shrink-0" /> : <ChevronDown className="w-4 h-4 text-slate-500 shrink-0" />}
        </button>
      </div>
